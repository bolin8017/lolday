...
const schema = z.object({
  discord_user_id: z.string()
    .regex(/^\d{15,20}$/u, "Discord IDs are 15–20 digits (enable Developer Mode in Discord, then right-click your name → Copy User ID)")
    .or(z.literal("")),
});
...
export function DiscordIdForm() {
  ...
  const onSubmit = handleSubmit(async (v) => {
    const next = v.discord_user_id.trim() === "" ? null : v.discord_user_id.trim();
    ...
    <form className="space-y-3" onSubmit={onSubmit}>
      <div>
        ...
        {errors.discord_user_id && (
          <p className="text-xs text-destructive">{errors.discord_user_id.message}</p>
        )}
        <p className="mt-1 text-xs text-muted-foreground">
          Set this to receive direct @mentions in <code>#lolday-alerts-events</code>.
          Leave blank to opt out of pings (you'll still see your name as plain text).
        </p>
      </div>
