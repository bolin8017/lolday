import { useClusterGpuStatus, useClusterQueueDepth } from "@/api/queries/cluster";
    ...
    <Card>
      <CardContent className="flex items-center gap-4 py-3 text-sm">
        ...
        {gpu.isLoading || queue.isLoading ? (
          ...
          <span className="text-muted-foreground">cluster status unavailable</span>
        ) : (
          <>
            <span>
              <strong>{gpu.data?.in_use ?? 0}/{gpu.data?.total ?? 0}</strong> GPUs allocated
              <span className="text-muted-foreground"> ({gpu.data?.idle ?? 0} idle)</span>
            </span>
