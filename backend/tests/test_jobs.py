...
@pytest.mark.asyncio
async def test_submit_job_rejects_target_override(
    user_client, seed_detector_version, seed_dataset
):
    """Phase 11c params guard: _target_ override is forbidden (would RCE)."""
    dv_id = await seed_detector_version()
    train_ds = await seed_dataset(name="tr-ds")
    test_ds = await seed_dataset(name="te-ds")
    ...
    assert "_target_" in r.json()["detail"]


@pytest.mark.asyncio
async def test_submit_job_rejects_paths_override(
    user_client, seed_detector_version, seed_dataset
):
    """Phase 11c params guard: paths.* override is platform-controlled."""
    dv_id = await seed_detector_version()
    train_ds = await seed_dataset(name="tr-ds")
    test_ds = await seed_dataset(name="te-ds")
...
@pytest.mark.asyncio
async def test_submit_job_accepts_simple_hyperparam_override(
    user_client, seed_detector_version, seed_dataset
):
    """Phase 11c params guard: simple hyperparameter overrides pass."""
    dv_id = await seed_detector_version()
    train_ds = await seed_dataset(name="tr-ds")
    test_ds = await seed_dataset(name="te-ds")
