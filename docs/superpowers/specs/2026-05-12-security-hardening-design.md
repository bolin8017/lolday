...
| H-16                    | HIGH     | `charts/lolday/templates/ingress.yaml:25-31` `/mlflow/` route — Traefik middleware method allowlist: `GET, HEAD, OPTIONS` for any SSO user; `POST, PATCH, DELETE` only for admin allowlist.[^h16-shipped]                                                                                                                                              |
...
[^h16-shipped]: H-16 shipped via the backend ForwardAuth handler (`backend/app/routers/mlflow_authz.py:240,253-257`) reading `X-Forwarded-Method` from Traefik, not via a separate Traefik headers middleware as the spec text described. Behaviour is equivalent — `POST/PATCH/DELETE` are admin-only — but the enforcement layer differs. See `docs/phase-history/2026-05-14-security-audit-findings.md` H-16 footnote for the full divergence note.
