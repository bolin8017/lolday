...
- **`lolday-jobs-quota` 拿掉 GPU 軸**（chart 目前只設 `requests.nvidia.com/gpu`，沒有 `limits` 那條；移掉 requests 即可）。Volcano queue capability 是唯一的 GPU gatekeeper。
- **Volcano scheduler 加 `sla` plugin** via Volcano sub-chart 官方提供的 helm value `volcano.custom.scheduler_config_override`（`charts/lolday/charts/volcano-1.14.1.tgz` 的 values.yaml 第 64–82 行記載示範）。整個 scheduler config 用單一字串 override，sla 條目 inline 進去。**不寫 post-install hook**、不另建 ConfigMap patcher Job。
...
1. **6a — `lolday-jobs-quota` 移除 GPU 軸**：`charts/lolday/templates/jobs-quota.yaml` 拿掉 `requests.nvidia.com/gpu` 那一行。
2. **6b — Volcano scheduler 加 `sla` plugin**：在 `charts/lolday/values.yaml` 的 `volcano.custom.scheduler_config_override` 字串裡 inline 加 sla（tier 1，與 `priority` / `gang` / `conformance` 同層），global `sla-waiting-time: 10m`。
3. **6c — smoke test**：`tests/2026-05-05-phase6-fifo-smoke.sh` 自動化 Test D 場景，斷言 d-BIG 在 d-SMALL 之前 schedule（測試會暫降 sla-waiting-time 到 30s 跑完還原）。
4. **6d — 文件更新**：`docs/architecture.md` §10 Phase 1 / §10 Phase 2 描述同步；`.claude/rules/charts-and-helm.md` 補 sla plugin / scheduler_config_override 的條目；`docs/conventions.md` 不變（沒新慣例）。
...
`charts/lolday/templates/jobs-quota.yaml` diff（chart 只設了 requests 那條，沒 limits.nvidia.com/gpu）：
...
### 6.2 6b — Add `sla` plugin via `volcano.custom.scheduler_config_override`

Volcano sub-chart 在 `charts/lolday/charts/volcano-1.14.1.tgz` 的 `values.yaml` 第 64–82 行明寫了 `custom.scheduler_config_override` 這個官方 escape hatch — 用 helm value 餵一段完整 scheduler config 字串，sub-chart 的 `templates/scheduler.yaml` 第 18–23 行直接把它寫進 `lolday-scheduler-configmap.data."volcano-scheduler.conf"`，**取代** sub-chart 自帶的預設。

`charts/lolday/values.yaml` 在現有 `volcano.custom` block 加：

```yaml
volcano:
  enabled: true
  custom:
    metrics_enable: false # already there (Phase 9.5)
    # Phase 6 (2026-05-05): replace Volcano sub-chart default scheduler
    # config to add `sla` plugin in tier 1 — gives aging-based admission
    # / pipeline boost so large GPU jobs are not perpetually leapfrogged
    # by smaller jobs. See docs/superpowers/specs/2026-05-05-gpu-fifo-
    # anti-starvation-design.md §6.2 for layout, §5.3 for sla-waiting-
    # time tuning.
...
唯一改動相對於 sub-chart 預設：tier 1 plugins 末尾加 `name: sla` + `arguments.sla-waiting-time: 10m`。

**為什麼選這條路、不寫 post-install patch hook**：

1. `scheduler_config_override` 是 Volcano 官方在 sub-chart values 暴露的官方 escape hatch — 寫 helm value 是 mainstream 做法，自寫 ConfigMap-patch hook 是繞遠路。
2. 不需要新 ServiceAccount / Role / RoleBinding 給 patch Job。
3. `helm upgrade` 自動把改動套到 ConfigMap、sub-chart 自己的 template logic 處理 scheduler Pod 滾動（依賴 ConfigMap hash 自動 rollout）。
4. `helm rollback` 直接還原此 value、scheduler ConfigMap 自動退回上版。

**為什麼可以接受「整段 scheduler config blob 寫死在 values.yaml」**：

- 我們是第一次需要改 scheduler config；以前所有 plugin（drf / proportion / priority / gang / ...）都是 sub-chart 預設。
- helm value 不能 templated（values 在 template 階段之前就 resolve），所以個別 plugin 不能拆參數。
- 如果未來 Volcano 1.15 預設 plugin order 變了，operator 要手動 reconcile — 這個成本記在 §10 open question #3。

### 6.3 Operator workflow when tuning sla-waiting-time

Operator 想把 sla 從 `10m` 調 `5m`：

1. 直接編 `charts/lolday/values.yaml` 把 `sla-waiting-time: 10m` 改 `5m`。
2. `bash scripts/deploy.sh`。
3. helm 把 `lolday-scheduler-configmap.data."volcano-scheduler.conf"` 寫成新值；sub-chart `scheduler.yaml` 帶 `checksum/config` annotation pattern（Volcano 標準作法）會自動 rollout scheduler。
4. `kubectl -n lolday rollout status deploy lolday-scheduler` 確認 Ready。

不需要 manual `kubectl edit cm` 或 manual restart。

> Note: 如果發現 sub-chart **沒**自動加 checksum annotation（< Volcano 1.14 某版有此 bug），plan §6.5 會補一個 `kubectl rollout restart` 在 `scripts/deploy.sh` 的 post-deploy block。Phase 6 實作時要先驗證這條。
...
- `.claude/rules/charts-and-helm.md`：在 volcano-queue.yaml 的條目下方補一條，記下 `volcano.custom.scheduler_config_override` 是 lolday 第一次使用 Volcano sub-chart 的 escape hatch、修改前必看 spec §6.2。
...
| 模式                                  | 觸發                                              | 影響                                                                                       | 處理                                                                                            |
| ------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| sla plugin 配置錯（YAML invalid）     | values.yaml `scheduler_config_override` 寫錯      | scheduler pod CrashLoopBackOff；所有 vcjob 卡 Pending                                      | `helm template` 在 deploy 前抓；deploy.sh 跑 `kubectl rollout status` 偵測；operator 看 events  |
| sla 啟用後仍有大 job 卡 > 10m         | cluster 完全 packed、無 job natural finish        | sla 等於 no-op；不丟 job 但 user 看到等很久                                                | Phase 4 alert (`JobsPendingHigh`) 觸發；operator 介入決定 manual cancel 或調 `sla-waiting-time` |
| `lolday-jobs-quota` GPU 軸誤恢復      | 某 phase 升級漏掉 6a 的 chart 改動                | 回退到 Test B 的 admission race；sla 失效                                                  | 6c smoke test 在每次 deploy 跑；smoke 內含「resourcequota 不應有 GPU 軸」斷言                   |
| sub-chart Volcano 升級換掉預設 plugin | helm 升 Volcano 1.14 → 1.15                       | 我們的 override 與新版預設不對齊（漏掉新 plugin / 多了過時的）                             | 6c smoke 失敗；operator 對照新版 default 後手動 reconcile override 字串                         |
...
- `helm template charts/lolday | grep -B 2 -A 4 'lolday-jobs-quota' | grep -c 'nvidia.com/gpu'`：應為 0。
- `helm template charts/lolday | grep -A 1 'name: sla'`：應印出 sla plugin block（從 ConfigMap data section render 出來）。

### 8.2 Post-deploy — smoke

- `bash tests/2026-05-05-phase6-fifo-smoke.sh`：跑 Test D 場景 + 斷言 BIG 先於 SMALL（這時 admission 不再卡，純 sla 救援）。
- `kubectl -n lolday get cm lolday-scheduler-configmap -o jsonpath='{.data.volcano-scheduler\.conf}' | grep -A 2 'name: sla'`：確認 sla 在 tier 1，arguments 含 `sla-waiting-time`。
- `kubectl -n lolday-jobs get resourcequota lolday-jobs-quota -o jsonpath='{.spec.hard}' | grep -c 'nvidia.com/gpu'`：應為 0（沒有 GPU 維度）。
...
```bash
helm rollback lolday <prev-revision>
```

因為 Phase 6 的所有改動都在 chart values + chart template，helm rollback 會把 `lolday-scheduler-configmap.data` 還原成上一版（含 sub-chart 預設 scheduler config）、`lolday-jobs-quota` 還原 GPU 軸。Sub-chart 的 ConfigMap checksum annotation 會觸發 scheduler rollout。

**次選 — emergency manual** （helm rollback 失敗時）：

1. **6b 應急回退**：`kubectl -n lolday edit cm lolday-scheduler-configmap`，手動把 sla 三行刪掉；`kubectl -n lolday rollout restart deploy lolday-scheduler`。
2. **6a 應急回退**：`kubectl -n lolday-jobs patch resourcequota lolday-jobs-quota --type=json -p='[{"op":"add","path":"/spec/hard/requests.nvidia.com~1gpu","value":"2"}]'`。
3. 隔天手動跑 `helm rollback lolday <prev-revision>` 把 chart 跟 cluster state 對齊（防止下次 deploy.sh 又把 sla 補回來）。
