...
def compute_real_gpu_state() -> GPUState:
    ...
    GPU activity. Cached for `GPU_SIGNAL_CACHE_TTL_SECONDS` (default 10s).
...
`@cached(TTLCache(maxsize=1, ttl=GPU_SIGNAL_CACHE_TTL_SECONDS))` 與既有 `cluster_status._gpu_cache` 一致。`/cluster/gpu-status` 在 dashboard polling cycle（每 15s）下會 hit cache，不會把 Prom 打爆。
...
| `GPU_SIGNAL_QUERY_TIMEOUT_SECONDS`  | `float` | `5.0`                                       | httpx timeout                                  |
| `GPU_SIGNAL_CACHE_TTL_SECONDS`      | `int`   | `10`                                        | TTL cache。與 `cluster_status._gpu_cache` 一致 |
