...
30. **Phase 2 deferred follow-ups (partial close 2026-05-16; heavy testcontainers tier still pending)** — Phase 3 (`feat/test-architecture-phase-3`) closes the frontend-side deferrals: D2.6 #20-21 (per-route MSW integration tests for jobs route + JobSubmitForm dependencies) shipped via `frontend/tests/integration/routes/jobs.test.tsx` + `forms/JobSubmitForm.flow.test.tsx`, and D2.7 visual snapshots (RJSF / Sidebar / PageHeader) shipped as `frontend/tests/visual/{rjsf_form,sidebar,page_header}_snapshots.spec.ts` (initially `test.skip` until operator runs `playwright test --update-snapshots` to seed baselines via the new live-stack fixture). **Still deferred to Phase 4**: (a) D2.3 #9 real-MLflow ACL multi-user via testcontainers MLflow, (b) D2.3 #12 audit-log durability on real Postgres via testcontainers, (c) D2.4 #13 JWKS reflector via uvicorn-served `well-known/jwks` — these need a Docker daemon in the dev environment + testcontainers fixture scaffolding that's better timed with Phase 4's broader test-telemetry + heavy-tier expansion. Owner: backend (Phase 4 plan).
