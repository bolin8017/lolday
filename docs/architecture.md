...
26. **Post-program scheduled gates (2026-05-15 review)** — Two reminders on the platform calendar from `docs/phase-history/2026-05-15-security-post-program-review.md`. (a) **PSS `enforce=restricted` promotion** on `lolday` / `lolday-jobs` / `monitoring` / `trivy-system` after the 3-day audit window — issue #186 fires 2026-05-18. Runbook: `docs/runbooks/pss-label-promotion.md`. `lolday-builds` stays at baseline by design. Helm-unittest invariant (added 2026-05-15 in #193): `charts/lolday/tests/pss_test.yaml` pins `lolday-jobs` enforce label at `baseline` via the "lolday-jobs namespace enforces baseline PSS" test. The promotion PR must update the expected value to `restricted` or the suite regresses. (b) **Harbor cosign Audit→Enforce promotion** after 7-day audit — issue #187 fires 2026-05-22, see item 25(b). Both are operator-decision gates; the chart change to land each promotion is a one-line label flip or a `kubectl patch`. Helm-unittest invariant: see §10.25(b). Owner: platform.
...
31. **Spidey Heartbeat positive-heartbeat (deferred from 2026-05-16 monitoring-NP hotfix)** — Spidey Heartbeat (channel `1495780321239502919`) has been silent since the 2026-05-10 alerting redesign because `deadmans-switch` is the SRE dead-man-switch pattern (failure-only POST to `webhook-url-critical` → Captain Hook). The `docs/operations.md` description has been corrected to match implementation; promotion to a true positive-heartbeat sink is a clean, isolated follow-up: (a) operator provisions a Spidey Heartbeat webhook URL (Discord → channel → Integrations → Create Webhook); (b) add `DISCORD_WEBHOOK_URL_HEARTBEAT` to `~/.lolday-secrets.env` and `.lolday-secrets.env.example`; (c) extend `scripts/deploy.sh` to push it into the `alertmanager-discord` Secret as `webhook-url-heartbeat`; (d) extend `charts/lolday/templates/monitoring/deadmans-switch.yaml` with a second env `DISCORD_HEARTBEAT_URL` referencing the new key; (e) extend `charts/lolday/files/deadmans_switch/check.py` so `main()` POSTs a short success ping to `DISCORD_HEARTBEAT_URL` after a successful Watchdog check, skipping with a `print()` line if the env is unset (graceful degrade); (f) extend `backend/tests/integration/services/test_deadmans_switch_check.py` with success-path coverage. Effort: ~1 PR. Risk: low (graceful degrade). Owner: platform.

32. **Trivy CRITICAL CVE backlog across 6 cluster images (2026-05-16)** — Trivy Operator currently reports 6 images with CRITICAL CVEs: `prodrigestivill/postgres-backup-local:16` (6 CVE — most likely glibc / openssl in the Debian base; check whether a newer tag landed); `goharbor/nginx-photon:v2.15.0` (1 CVE); `goharbor/harbor-portal:v2.15.0` (1 CVE); `goharbor/harbor-db:v2.15.0` (1 CVE); `goharbor/redis-photon:v2.15.0` (1 CVE); `minio/minio:RELEASE.2024-12-18T13-15-44Z` (1 CVE). Each requires independent CVE → fix-version research (the Harbor releases are coupled; MinIO and pg-backup are independent). Refresh the digest pins in `values.yaml` once an upstream tag with the fix lands. Driver: Trivy dashboard. Effort: 1 PR per image family. Owner: platform.
