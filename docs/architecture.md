...
36. **`lolday` ns PSS `enforce=restricted` deferred — sub-chart pods non-compliant** — Discovered while closing §10 #26(a) on 2026-05-18: a runtime `kubectl label ns lolday pod-security.kubernetes.io/enforce=restricted --overwrite --dry-run=server` emits warnings for five workloads in the release ns (audit-mode misses them because the audit-violations annotation only lands at pod-admission time, and these pods predate the `audit=restricted` label added in chart 0.24.0). Affected (live audit, 2026-05-18, post PR #272):
    ...
    - `lolday-admission` / `lolday-controllers` / `lolday-scheduler` (**Volcano** sub-chart `~1.14.1` — earlier draft of this entry mis-labelled these as Kyverno controllers; the actual Kyverno controller pods `kyverno-{admission,background,cleanup,reports}-controller` are PSS-restricted compliant out of the box). The blocker is `containers[*].securityContext.capabilities.add: ["DAC_OVERRIDE"]` on all three Volcano workloads; PSS `restricted` allows only `["NET_BIND_SERVICE"]` in `capabilities.add`. Upstream investigation (2026-05-18 web search of Volcano docs + chart): `DAC_OVERRIDE` is documented as required by the `volcano-admission-init` cert-generation Job (`gen-admission-secret.sh` writes the webhook certs to filesystem paths owned by root). The chart's `custom.default_csc` adds `DAC_OVERRIDE` blanket-style to ALL containers — but the three running main containers (admission webhook, controller-manager, scheduler) don't visibly use it at runtime; they only handle K8s API requests. **Hypothesis**: setting per-controller overrides (`custom.admission_main_csc`, `custom.scheduler_main_csc`, `custom.controller_main_csc`) to a copy of `default_csc` minus `DAC_OVERRIDE` would let main pods meet PSS `restricted` while keeping the cert-gen init Job (which uses `agent_init_csc` separately) working. **Verification still gated** on (a) confirming via live cluster runtime that none of the three main containers actually touches a path needing `DAC_OVERRIDE`, AND (b) the cert-gen init Job is separate from the runtime pods (live audit confirmed no init containers on the three running pods, so this looks safe). Next session: ship a chart values patch behind a flag (`volcano.custom.main_csc_drop_dac_override: true`) defaulting to `false`, flip on with operator awareness, watch logs for permission denials. Until shipped, this is the load-bearing blocker for the `lolday`-ns promotion.
