...
14. **`frontend/src/api/schema.gen.ts` drift detection (partial close 2026-05-16)** — the file is generated from a running backend's `/openapi.json` via `pnpm gen-api-types`, but PR #69 hand-stitched `detector_defaults` on `JobRead` and a later PR hand-stitched `"gpu1"` into the `ResourceProfile` enum. **Phase 2 D2.8 (commit `568ef7a`)** ships `frontend/tests/contract/schema_gen_drift.test.ts` against a checked-in `frontend/tests/fixtures/openapi.snapshot.json` — the test fails loudly if either field disappears from the snapshot. **Remaining work (Phase 3 R5)**: split `schema.gen.ts` → `schema.gen.ts` (pure codegen) + `schema.handstitched.ts` (the two extensions) + `schema.ts` (merge + re-export), plus a CI step that boots a temp backend and runs `git diff --exit-code` after `pnpm gen-api-types`. The handstitched fields can be retired once the matching backend ships them natively (Phase 2 D2.8 contract becomes a smoke gate at that point).
...
30. **Phase 2 deferred follow-ups (heavy testcontainers tier + frontend route-render integration)** — Phase 2 ships D2.1-D2.10 with three explicit deferrals: (a) D2.3 Task 9 (real-MLflow ACL multi-user via testcontainers MLflow), (b) D2.3 Task 12 (audit-log durability on real Postgres via testcontainers), (c) D2.4 Task 13 (JWKS reflector via uvicorn-served `well-known/jwks`). Existing integration tier carries the contract; the heavy tier adds production-fidelity coverage that's valuable but not blocking. Also deferred to Phase 3: D2.6 Tasks 20-21 (per-route MSW integration tests) need `createMemoryRouter` scaffolding against the file-based `@tanstack-router` routes — layered on top of the MSW smoke shipped in commit `c81e878`. D2.7 visual snapshots (RJSF/Sidebar/PageHeader) similarly need a stable dev-server fixture before baselines can be generated. Owner: backend + frontend (Phase 3 plan).
