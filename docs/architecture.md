...
33. **Residual Trivy CRITICAL CVEs after the 2026-05-16 image-bump round** — 6 CRITICAL CVEs remain across two images after §10 #32 closed; the `TrivyCriticalCVE` warning alert will keep firing until each is addressed.
    ...
    Owner: platform. The `TrivyCriticalCVE` Spidey-Warnings ping was originally going to be kept firing as an audit signal, but the operator pushed back on the recurring Discord noise. 2026-05-16 follow-up routes all five CVE IDs through the existing `trivy-operator.trivy.ignorePolicy` Rego `residual_cves` set in `charts/lolday/values.yaml` (same mechanism the chart already uses for 8 other documented residuals). The Rego predicate drops `sum(trivy_image_vulnerabilities{severity="Critical"})` to zero, silencing the recurring Spidey Warnings ping. The underlying VulnerabilityReport CRs stay in-cluster (`kubectl get vulnerabilityreport -A` remains the audit source-of-truth) so the audit trail isn't lost — only the alert metric is filtered. If a NEW CRITICAL CVE appears (different CVE ID), the count goes 0 → 1 → 10m later the alert fires again — so the suppression is per-CVE-ID-targeted, not blanket. Re-evaluate by 2026-08-14: prune the entries from `residual_cves` once upstream images ship the libcrypto / libxml2 fixes, or once a new MinIO release retires the OIDC / LDAP-backend CVEs.
...
36. **`lolday` ns PSS `enforce=restricted` deferred — sub-chart pods non-compliant** — Discovered while closing §10 #26(a) on 2026-05-18: a runtime `kubectl label ns lolday pod-security.kubernetes.io/enforce=restricted --overwrite` emits warnings for five running pods that ride sub-charts vendored into the release ns (audit-mode misses them because the audit-violations annotation only lands at pod-admission time, and these pods predate the `audit=restricted` label added in chart 0.24.0). Affected:
    - `loki-0` (StatefulSet `loki`, sub-chart `loki ~7.0.0`) — missing `seccompProfile`.
    - `lolday-admission` / `lolday-background` / `lolday-cleanup` / `lolday-reports` (Kyverno controllers) — unrestricted `capabilities`.
    - `lolday-alloy` DaemonSet — multiple violations (`allowPrivilegeEscalation`, capabilities, restricted volume types, `runAsNonRoot`, `seccompProfile`).
    - `lolday-minio` Deployment — multiple violations (`allowPrivilegeEscalation`, capabilities, `runAsNonRoot`, `seccompProfile`).
    ...
    Resolution path: patch each sub-chart's `values.yaml` `podSecurityContext` / `securityContext` block (`seccompProfile.type: RuntimeDefault`, drop ALL capabilities, set `runAsNonRoot: true` + `runAsUser: <uid>`) — for the four chartable cases. For node-exporter, the `restricted` profile is unreachable by design; either keep `lolday` at `baseline` permanently, accept a per-pod `pod-security.kubernetes.io/enforce-version: latest` waiver, or relocate node-exporter to a dedicated `monitoring-host` ns. Once the four chartable pods are clean, retry the lolday-ns promotion and update `scripts/deploy.sh` to add `pod-security.kubernetes.io/enforce=restricted` to the `kubectl label ns lolday` call. Owner: platform.

37. **schemathesis 4 migration** — Dependabot PR #249 tried to bump `schemathesis` 3.39.16 → 4.18.5 and CI failed loudly (closed on 2026-05-18 with a migration-findings comment). The bump is real work, not a fast-lane PR. Three mechanical changes: (a) delete `schemathesis.experimental.OPEN_API_3_1.enable()` in `backend/tests/contract/conftest.py` (the `experimental` namespace is gone in v4; OAS 3.1 is native), (b) rename `schemathesis.from_asgi(...)` → `schemathesis.openapi.from_asgi(...)` in the same conftest, (c) rename `schemathesis.from_pytest_fixture("schema")` → `schemathesis.pytest.from_fixture("schema")` in the three `backend/tests/contract/openapi/test_schemathesis_*.py` files. The decision-needing change: v4 ships a new `RejectedPositiveData` check (`schemathesis.openapi.checks.positive_data_acceptance`) that flags every schema-compliant request the server rejects with `422`. FastAPI auto-injects `422` only on operations with query params or a request body, so most lolday endpoints regress (local probe: 22/32 contract subtests fail). Pick one: (i) add `responses={422: {...}}` to every operation that lacks it (laborious but correct — same pattern as Phase 2 D2.3 Task 8 / §10.28), or (ii) disable `positive_data_acceptance` via a conftest hook (loses the new safety net but is mechanical). Transitive cascade: schemathesis 4's lockfile forces `pytest 8.4.2 → 9.0.3` (closes the open `pytest` GHSA-medium advisory automatically) and `pyrate-limiter 3.9.0 → 4.1.0` (also a major in a transitive). The pytest 9 cascade has its own breaking-change list (CLI removals, fixture changes) and must not be folded into the same PR without a separate review. Owner: backend. Auto-memory note: [[schemathesis-4-migration]].

38. **Helper-image Dockerfiles install fresh from `pyproject.toml`, not the lockfile** — Discovered while shipping PR #263 on 2026-05-18. `charts/lolday/helpers/job-helper/Dockerfile` copies `pyproject.toml` (not `uv.lock`) and runs `pip install --no-cache-dir .`, so Docker's layer cache hits with the same input and a `uv lock --upgrade-package <transitive>` bump produces a byte-identical image — same `sha256:` digest, same cosign signature. Net effect today: Dependabot scan against `job-helper/uv.lock` closes when the lock is bumped (the file content changes) but the runtime libraries inside the image don't actually move. PR #263 paid the operator round-trip (rebuild + sign + push + helpers.lock refresh) for what amounts to a Dependabot-visibility fix only. Mainstream pattern (already used in `backend/Dockerfile` and `charts/lolday/helpers/build-helper/Dockerfile` via `requirements.txt` with `--require-hashes`) is to make the lock authoritative: switch the job-helper Dockerfile to `uv sync --frozen --no-dev` (or `uv pip compile pyproject.toml --generate-hashes -o requirements.txt` + `pip install --require-hashes -r requirements.txt`, matching build-helper). Either path triggers a real digest bump on the next rebuild — cosign re-signs by digest so Kyverno admission picks it up naturally, and the runtime image actually carries the patched transitive deps. Scope is one Dockerfile + one operator rebuild; tracked here so the next security follow-up that hits a job-helper transitive doesn't pay the same round-trip for Dependabot-visibility only. Owner: platform / operator.
