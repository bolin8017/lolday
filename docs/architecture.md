...
14. ~~**`frontend/src/api/schema.gen.ts` drift detection**~~ — fully resolved 2026-05-16 in Phase 3 D3.8 / R5 (`feat/test-architecture-phase-3`). Tri-file split: `schema.gen.ts` (100% openapi-typescript output, no hand-edits), `schema.handstitched.ts` (initially the two extensions — `JobRead.detector_defaults` + `ResourceProfile.gpu1` — as the single source of truth), `schema.ts` (TypeScript-level merge + re-export barrel; call sites import from here). `frontend/tests/contract/schema_gen_drift.test.ts` now enforces both the snapshot side (Phase 2 D2.8 invariant) AND the structural side. `frontend.yml` boots a uvicorn instance, runs `pnpm regen-openapi-snapshot`, and `git diff --exit-code`s the result so backend drift fails the PR loud. **Retirement (2026-05-19, `chore/retire-schema-handstitched`):** the backend `/openapi.json` now declares both `JobRead.detector_defaults` and `ResourceProfile.gpu1` natively, so `schema.gen.ts` was re-baselined via `pnpm gen-api-types` (this also surfaced ambient drift from `/api/v1/internal/*` migrating to the internal_app on port 8001 and `/api/v1/dev/seed-fixtures` from D3.3 dev-seed). `schema.handstitched.ts` is kept as an empty-type identity passthrough (`JobReadHandstitchedExtensions = unknown`, `ResourceProfileHandstitched = never`) so `schema.ts`'s merge stays a no-op (`X & unknown ≡ X`, `X | never ≡ X`). The structural contract test now asserts `schema.gen.ts` carries the formerly-handstitched fields natively. Future natively-shipped extensions (if any) can re-populate the handstitched types without disturbing `schema.ts`.
...
18. ~~**`@microlink/react-json-view` (frontend, JsonTreeView)**~~ — resolved 2026-05-20 in `feat/json-tree-view-lite`. Swapped to `react-json-view-lite` 2.5.0 (actively maintained under the `anyroad` org, ~5.5 KB minified vs the lodash-bloated fork). `JsonTreeView.tsx` now switches `defaultStyles` / `darkStyles` based on the resolved theme — the library's hard-coded `darkStyles.container` background (`rgb(0, 43, 54)`) is dropped via `{ ...darkStyles, container: "" }` so the shadcn `bg-card` token shows through. The lib has no built-in copy button (was `enableClipboard` on the old fork), so the wrapper now ships its own floating shadcn `Button` with a `lucide-react` `Copy → Check` icon swap on click, calling `navigator.clipboard.writeText(JSON.stringify(value, null, 2))`. Public API of `<JsonTreeView value collapsed copyable />` is unchanged; the two call sites (`ResolvedConfigCard.tsx`, `_authed.detectors.$id.tsx::ManifestSection`) keep working. The lib's `shouldExpandNode((level) => boolean)` mirrors the old fork's `collapsed: number | boolean` semantics (`level < N` for the numeric form). Unit-test rewrite covers nine cases including theme switching, collapse depth, and the new copy-button flow. Owner: frontend.
...
22. ~~**Predict / Evaluate emergency path when training detector_version is retired**~~ — resolved 2026-05-19 in `feat/model-version-is-runnable`. Backend extends `ModelVersionRead` with a derived `is_runnable: bool` populated via the existing 5-site `DetectorVersion` JOIN (`is_runnable = (status == DetectorVersionStatus.ACTIVE)`); the helper `_model_version_to_read` accepts `detector_version_status` as a sixth arg so each call site stays explicit about the joined columns. Frontend `InferenceSubForm` reads `is_runnable` per option and renders the retired entries `disabled` with an i18n short label (`training version retired` / `訓練版本已退役`) inline and the full reason (`Training detector version has been retired. Retrain this model...`) as the `title` tooltip. Backend job-submit still rejects with 422 if the value races between page-load and submit — the dropdown disable is the up-front UX repair on top of the existing repair path in `JobSubmitForm.tsx::submit()`. Mainstream practice remains "retrain rather than hot-swap runtimes" (MLflow Model Registry / SageMaker / BentoML). Backend test: `backend/tests/integration/services/test_models_registry.py::test_model_version_read_is_runnable_tracks_dv_status` covers ACTIVE → True, RETENTION_PRUNED → False on both the list and single-version endpoints. Frontend test: `frontend/tests/unit/components/InferenceSubForm.test.tsx::disables model versions whose training detector version was retired`. Owner: backend + frontend.
...
36. **`lolday` ns PSS `enforce=restricted` deferred — sub-chart pods non-compliant** — Discovered while closing §10 #26(a) on 2026-05-18: a runtime `kubectl label ns lolday pod-security.kubernetes.io/enforce=restricted --overwrite --dry-run=server` emits warnings for five workloads in the release ns (audit-mode misses them because the audit-violations annotation only lands at pod-admission time, and these pods predate the `audit=restricted` label added in chart 0.24.0). Affected (live audit, 2026-05-18, post PR #272):
    ...
    - `lolday-alloy` DaemonSet (Grafana Alloy `~1.8.0`) — intrinsically `baseline` (parallel to node-exporter): log collection requires hostPath volumes for `/var/log` and `/var/lib/docker/containers`, and hostPath volumes are forbidden under PSS `restricted` by design. Defense-in-depth hardening applied (sub-chart values, no chart-version bump): pod-level `alloy.global.podSecurityContext.seccompProfile.type: RuntimeDefault` + container-level `alloy.alloy.securityContext.{allowPrivilegeEscalation: false, capabilities.drop: [ALL], seccompProfile.type: RuntimeDefault}` on the `alloy` container. `runAsNonRoot` is deliberately not set on the `alloy` container — the upstream `grafana/alloy` image reads `/var/log/pods/*.log` which are root-owned on standard K3s + Docker nodes; flipping to non-root would break log collection. The `config-reloader` sidecar — no host volumes, only watches the `/etc/alloy` ConfigMap mount — now runs with the full PSS `restricted` invariant set: `runAsNonRoot: true`, `runAsUser: 65534`, `runAsGroup: 65534`, `readOnlyRootFilesystem: true`, alongside the existing `allowPrivilegeEscalation: false`, `capabilities.drop: [ALL]`, `seccompProfile.type: RuntimeDefault`. The numeric UID/GID matches the upstream `prometheus-config-reloader` Dockerfile's `USER nobody`. The sidecar individually satisfies PSS `restricted` even though the parent ns stays at `baseline`.
    ...
    - `lolday-prometheus-node-exporter` DaemonSet — intrinsically `baseline`: metric collection needs `hostNetwork` / `hostPID` / `hostPort` + the `/host/{proc,sys,root}` hostPath volumes, which are forbidden under PSS `restricted` by design. Defense-in-depth hardening applied (kps sub-sub-chart values under `kps.prometheus-node-exporter`, no chart-version bump): pod-level adds `seccompProfile.type: RuntimeDefault` on top of the upstream `runAsNonRoot: true` + uid 65534 defaults; container-level extends the upstream `readOnlyRootFilesystem: true` with `allowPrivilegeEscalation: false`, `capabilities.drop: [ALL]`, `seccompProfile.type: RuntimeDefault`. Same intrinsic-baseline rationale as lolday-alloy above — pin at `baseline` (per-pod label or side-ns).

    Resolution path: two pods (Alloy + node-exporter) are intrinsically incompatible with PSS `restricted` because of hostPath / host-namespace requirements that are not negotiable for log + host-metric collection. The `lolday` ns therefore stays at `baseline` permanently unless the operator chooses to (a) relocate node-exporter + Alloy to a dedicated `monitoring-host` ns at `baseline` while promoting `lolday` to `restricted`, or (b) accept a per-pod `pod-security.kubernetes.io/enforce-version: latest` waiver on those two DaemonSets. The defense-in-depth hardening shipped in this round narrows the blast radius for both pods at `baseline` (capability drop + no privilege escalation + RuntimeDefault seccomp) so the gap between `baseline` and `restricted` is mostly the hostPath / host-namespace volumes themselves, not behavioural laxity. Owner: platform.
