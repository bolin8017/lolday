...
28. **`GET /api/v1/users/me` OpenAPI is missing a 422 response entry** — Phase 1 schemathesis contract testing surfaced this; `backend/tests/contract/openapi/test_schemathesis_users_me.py` carries an inline `pytest.xfail()` documenting the gap. The route is declared as `@router.get("/me", response_model=UserRead)` in `backend/app/routers/users_me.py` with no explicit `responses=` entry. FastAPI does not auto-populate `responses[422]` for endpoints without request body / query params / path params, so the OpenAPI schema declares the endpoint as 200-only while the runtime can still raise `RequestValidationError`. Fix: add `responses={422: {"model": HTTPValidationError}}` (or the project's preferred error envelope — see `routers/internal.py:67` for the `HTTPException(422, detail=…)` precedent) to the `read_me` route decorator and remove the schemathesis xfail. Owner: backend.

29. **`chart-e2e.yml` k3d cluster lacks Traefik 3 CRDs** — the workflow was introduced in #193 (Phase 1) and has never observed a green `push` run on `main`. After #194 unblocked two pre-install errors (alembic pre-install hook timeout + `monitoring/quota.yaml` missing `monitoring.enabled` guard), the residual failure is `no matches for kind "IngressRoute" in version "traefik.io/v1alpha1"` at `helm install` time. Root cause: k3d v5.7.5 (pinned in the workflow) bundles k3s 1.30 → Traefik 2.x which serves the legacy `traefik.containo.us/v1alpha1` API group, not the chart's `traefik.io/v1alpha1`. Two candidate fixes: (a) bump `TAG=v5.7.5` to a k3d build that ships k3s 1.31+ (Traefik 3.x) — closer to mainstream, smoke validates against the same Traefik major as production, but requires retesting k3d networking on newer k3s; (b) explicit `kubectl apply -f <pinned-traefik-3.x-CRD-bundle>` before the `helm install` step — smallest change but creates a chart-vs-cluster Traefik major mismatch. Decision deferred until someone owns the cluster-version upgrade thread. Owner: platform.
