...
- **Supply-chain policies (chart 0.24.0, 2026-05-15)** — `policies/verify-images.yaml` (Kyverno ClusterPolicy for `ghcr.io/bolin8017/lolday-*`, keyless via GHA OIDC, ships at `Enforce`) + `policies/verify-images-harbor.yaml` (key-based for `harbor.lolday.svc:80/lolday/*`, key sourced from Secret `{{ .Release.Namespace }}/cosign-harbor-pubkey` — `lolday` in practice, the ns where the Kyverno sub-chart actually runs; ships at `Audit` — promote in git via `kyverno.harborImageSignatureEnforce: true` in `values.yaml`, plumbed 2026-05-19, see `docs/architecture.md` §10 item 25b) + `policies/pss-baseline-audit.yaml`. Runbooks: `docs/runbooks/kyverno-bootstrap.md`, `docs/runbooks/kyverno-harbor-signing.md`.
...
- `alertmanager-rules.yaml` + `alertmanager-config-discord.yaml` — alerting rules + Discord receivers + 5 inhibition rules + per-severity routing. 20 alert rules total (alerting redesign 2026-05-10 + security-hardening P5/P6 added `LoldayAuthFailureSpike`, `LoldayRateLimitSpike`, `LoldayDiscordNotifyFailing` + 2026-05-17 audit added `LoldayDiscordNotifyDropped`). Receivers wire to two distinct Discord channels via Secret keys `webhook-url-critical` (Captain Hook, @here) and `webhook-url-warning` (Spidey Warnings, no @here). Routes: `severity={none,info} → null` (#206 / 2026-05-17 audit black-hole kps internals); `severity=critical → discord-critical`; `severity=warning → discord-warning`; default fallthrough = `discord-warning` (fail-loud for any unknown severity). See `docs/superpowers/specs/2026-05-10-alerting-redesign-design.md` + `2026-05-17-discord-notification-audit-design.md`.
- `deadmans-switch.yaml` — CronJob with TWO independent Discord webhooks, both **distinct** from the backend's `DISCORD_WEBHOOK_URL_EVENTS`. `DISCORD_URL` (mapped to `alertmanager-discord/webhook-url-critical` → Captain Hook) is the SRE dead-man-switch failure sink; missing this env causes fail-fast `RuntimeError` (CrashLoopBackOff surfaces the misconfig). `DISCORD_HEARTBEAT_URL` (mapped to `alertmanager-discord/webhook-url-heartbeat` → Spidey Heartbeat) is the positive-heartbeat sink; the env binding is `optional: true` so an unset Secret key gracefully degrades (`check.py` logs "skipped" and continues). POST failures on the heartbeat path are swallowed (never raise) so a transient heartbeat-sink hiccup cannot double-page via KubeJobFailed on top of Spidey Heartbeat silence. Wired 2026-05-16 in the §10 #31 follow-up to PR #201; see `charts/lolday/files/deadmans_switch/check.py`.
