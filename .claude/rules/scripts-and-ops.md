...
- **Supply-chain bootstrap** — `cosign-harbor-init.sh` (one-time per cluster: generates `~/.cosign/lolday-harbor.{key,pub}`, installs the public half as `lolday/cosign-harbor-pubkey` (release namespace — where the Kyverno sub-chart runs; the admission-controller SA has secret-read RBAC scoped to its own ns); `--force-new` for key rotation; runbook `docs/runbooks/kyverno-harbor-signing.md`).
